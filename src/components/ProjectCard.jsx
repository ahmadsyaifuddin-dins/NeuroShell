...
import { useToast } from '../hooks/useToast'; // Import hook toast
...
export default function ProjectCard({ proj, onToggle, onDelete, onCopy, onEdit, onViewLogs }) {
    ...
    // Buka modal jika ada backup key
    const handleOpenKeyModal = () => {
        if (proj.backupAppKey) {
            setIsKeyModalOpen(true);
        } else {
            error("NO BACKUP KEY SAVED YET.");
        }
    };

    // Fungsi eksekusi yang dipanggil oleh tombol "PROCEED & COPY" di dalam Modal
    const executeCopyAppKey = async () => {
        try {
            await navigator.clipboard.writeText(proj.backupAppKey);
            success('ORIGINAL APP_KEY COPIED TO CLIPBOARD.');
            setIsKeyModalOpen(false); // Tutup modal setelah sukses menyalin
        } catch (err) {
            error('FAILED TO COPY APP_KEY.');
        }
    };
    ...
    return (
        <>
                ...
                <div>
                    ...
                    <div className="flex justify-between items-start mb-6">
                        ...
                        <div className="flex gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300">
                            ...
                            {proj.backupAppKey && (
                                // Ubah onClick untuk memanggil handleOpenKeyModal
                                <button onClick={handleOpenKeyModal} className="p-2 hover:bg-yellow-500 hover:text-black rounded transition text-yellow-500" title="Copy Original Laravel APP_KEY">
                                    <Key size={16} />
                                </button>
                            )}
            ...
            {/* Panggil Modal di luar div card agar tidak terpengaruh z-index relatif dari card */}
            <AppKeyModal
                isOpen={isKeyModalOpen}
                onClose={() => setIsKeyModalOpen(false)}
                onConfirm={executeCopyAppKey}
            />
        </>
    );
}